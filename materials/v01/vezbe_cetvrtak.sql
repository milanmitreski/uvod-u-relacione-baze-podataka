...
-- Osnovna struktura:
-- 3 SELECT (projekcija) -- OBAVEZAN -> navodimo koje atribute zelimo da izdvojimo
-- 1 FROM -- OBAVEZAN -> navodimo iz koje tabele izvlacimo podatke
-- 2 WHERE (restrikcija) -- NIJE OBAVEZAN -> filtriramo redove (entitete) koje zelimo da prikazemo u zavisnosti od nekog uslova

-- SELECT -- ocekuje listu kolona ili * (sve kolone)
-- FROM -- ocekuje tabelu iz koje se podaci izvlace
-- WHERE -- ocekuje logicki uslov za filtriranje podataka
...
SELECT *
FROM DA.DOSIJE
WHERE POL = 'z' AND MESTORODJENJA = 'Beograd';
...
-- 10. Izdvojiti podatke o ispitnim rokovima koji nisu odrzani u 2015/2016, 2016/2017 ili
-- 2018/2019 skolskoj godini

SELECT *
FROM DA.ISPITNIROK
WHERE SKGODINA NOT IN (2015, 2016, 2018);

-- 11. Izdvojiti podatke o studentima koji su fakultet upisali 2015. godina,
...
-- 12. Izdvojiti nazive predmeta i njihovu cenu za samofin. studente izrazenu u dinarima, ako
-- jedan ESPB kosta 2000 dinara
...
SELECT NAZIV, ESPB * 2000 AS CENA_UPISA
FROM DA.PREDMET
WHERE ESPB * 2000 > 10000; -- jer se WHERE izvrsava pre SELECT-a

-- 14. Izdvojiti nazive predmeta i njihovu cenu za samofinansirajuce studente izrazenu u dinarima
-- Jedan ESPB kosta 2000 dinare. Izmedju kolone naziv i kolone cena dodati kolonu u kojoj ce za svaku
-- vrstu biti ispisano 'Cena u dinarima'

SELECT NAZIV, 'Cena u dinarima' AS OPIS , ESPB * 2000 AS CENA_UPISA
FROM DA.PREDMET;
