...
DROP TABLE IF EXISTS DA.PREDMET_STUDENT;

CREATE TABLE DA.PREDMET_STUDENT (
	IDPREDMETA INTEGER NOT NULL PRIMARY KEY ,
	STUDENT SMALLINT
);

INSERT INTO DA.PREDMET_STUDENT
SELECT IDPREDMETA, 5
FROM DA.PREDMETPROGRAMA
WHERE IDPROGRAMA = 103 AND VRSTA = 'obavezan';

SELECT * FROM DA.PREDMET_STUDENT;

MERGE INTO DA.PREDMET_STUDENT PS
USING (
    SELECT IDPREDMETA, COUNT(*) AS STUDENT
    FROM DA.ISPIT I
...
WHEN MATCHED THEN
    UPDATE SET PS.STUDENT = P.STUDENT
WHEN NOT MATCHED THEN
    INSERT VALUES (P.IDPREDMETA, P.STUDENT);
...
DROP TABLE IF EXISTS DA.STUDENT_PODACI;

CREATE TABLE IF NOT EXISTS DA.STUDENT_PODACI (
...
INSERT INTO DA.STUDENT_PODACI (INDEKS, BROJ_PREDMETA, PROSEK)
SELECT D.INDEKS, 10, 10.0
FROM DA.DOSIJE D
WHERE D.DATDIPLOMIRANJA IS NOT NULL
UNION
SELECT D.INDEKS, COUNT(*), AVG(OCENA+0.0)
FROM DA.DOSIJE D JOIN DA.ISPIT I ON (D.INDEKS = I.INDEKS)
WHERE D.DATDIPLOMIRANJA IS NULL AND I.STATUS = 'o' AND I.OCENA > 5
GROUP BY D.INDEKS
HAVING AVG(OCENA+0.0) > 8;

SELECT * FROM DA.STUDENT_PODACI;

MERGE INTO DA.STUDENT_PODACI SP
USING (
    SELECT D.INDEKS, D.DATUPISA, SS.NAZIV,
           COUNT(*) AS BROJ_PREDMETA, AVG(OCENA+0.0) AS PROSEK
...
WHEN MATCHED AND TMP.NAZIV = 'Diplomirao' THEN
    UPDATE SET SP.DATUPISA = TMP.DATUPISA
WHEN MATCHED AND TMP.NAZIV = 'Budzet' THEN
    UPDATE SET SP.BROJ_PREDMETA = TMP.BROJ_PREDMETA, SP.PROSEK = TMP.PROSEK
WHEN MATCHED AND LOWER(TMP.NAZIV) LIKE '%ispis%' THEN
    DELETE
WHEN NOT MATCHED AND LOWER(TMP.NAZIV) NOT LIKE '%ispis%' THEN
...
CREATE TRIGGER DA.BRISANJE_STUDENATA
BEFORE DELETE
ON DA.DOSIJE
REFERENCING
    OLD AS O
FOR EACH ROW
WHEN (
    O.DATDIPLOMIRANJA IS NOT NULL
)
BEGIN ATOMIC
    SIGNAL SQLSTATE '75000' ('Student je diplomirao, brisanje nije dozvoljeno.');
END;

INSERT INTO DA.DOSIJE (INDEKS, IDPROGRAMA, IME, PREZIME, IDSTATUSA, DATUPISA, DATDIPLOMIRANJA)
VALUES (20260001, 103, 'Ivan', 'Ivanovic', -2, '2017-06-01', '2025-01-01');

DELETE FROM DA.DOSIJE WHERE INDEKS = 20260001;

SELECT * FROM DA.DOSIJE WHERE INDEKS = 20260001;

DROP TRIGGER DA.BRISANJE_STUDENATA;
...
DROP TABLE IF EXISTS DA.BROJ_PREDMETA;

CREATE TABLE DA.BROJ_PREDMETA (
    BROJ SMALLINT NOT NULL PRIMARY KEY
);

INSERT INTO DA.BROJ_PREDMETA
SELECT COUNT(*)
FROM DA.PREDMET;

SELECT * FROM DA.BROJ_PREDMETA;

CREATE TRIGGER DA.PREDMET_UNESEN
AFTER INSERT
ON DA.PREDMET
FOR EACH ROW
BEGIN ATOMIC
    UPDATE DA.BROJ_PREDMETA SET BROJ = BROJ + 1;
END;

CREATE TRIGGER DA.PREDMET_OBRISAN
AFTER DELETE
ON DA.PREDMET
FOR EACH ROW
BEGIN ATOMIC
...
DROP TRIGGER DA.PREDMET_UNESEN;
DROP TRIGGER DA.PREDMET_OBRISAN;

CREATE TRIGGER DA.PROMENA_PREDMETA
...
DROP TRIGGER DA.PROMENA_PREDMETA;
...
DROP TABLE IF EXISTS DA.STUDENT_POLOZENO;

CREATE TABLE DA.STUDENT_POLOZENO (
    INDEKS INTEGER NOT NULL PRIMARY KEY,
    ESPB SMALLINT
);

DROP TABLE IF EXISTS DA.PREDMET_POLOZENO;

CREATE TABLE DA.PREDMET_POLOZENO (
    IDPREDMET INTEGER NOT NULL PRIMARY KEY,
    BROJSTUDENATA SMALLINT
);

INSERT INTO DA.STUDENT_POLOZENO (INDEKS, ESPB)
...
SELECT * FROM DA.STUDENT_POLOZENO;

MERGE INTO DA.STUDENT_POLOZENO SP
USING (
    SELECT INDEKS, SUM(P.ESPB) AS POLOZENO
...
ON SP.INDEKS = TMP.INDEKS
WHEN MATCHED THEN
    UPDATE SET SP.ESPB = TMP.POLOZENO
WHEN NOT MATCHED THEN
    INSERT (INDEKS, ESPB) VALUES (TMP.INDEKS, TMP.POLOZENO);

INSERT INTO DA.PREDMET_POLOZENO
SELECT IDPREDMETA, COUNT(*)
FROM DA.ISPIT I
WHERE I.OCENA > 5 AND I.STATUS = 'o'
GROUP BY IDPREDMETA;

SELECT * FROM DA.PREDMET_POLOZENO;

CREATE TRIGGER DA.POLOZEN_ISPIT
AFTER INSERT
ON DA.ISPIT
REFERENCING
    NEW AS N
...
BEGIN ATOMIC
    IF N.INDEKS IN (SELECT INDEKS FROM DA.STUDENT_POLOZENO) THEN
        UPDATE DA.STUDENT_POLOZENO SET ESPB = ESPB + (
            SELECT ESPB
            FROM DA.PREDMET
    ...
    IF N.IDPREDMETA IN (SELECT IDPREDMET FROM DA.PREDMET_POLOZENO) THEN
        UPDATE DA.PREDMET_POLOZENO
        SET BROJSTUDENATA = BROJSTUDENATA + 1
        WHERE IDPREDMET = N.IDPREDMETA;
    ELSE
        INSERT INTO DA.PREDMET_POLOZENO (IDPREDMET, BROJSTUDENATA)
        VALUES (N.IDPREDMETA, 1);
    END IF;
END;

SELECT * FROM DA.STUDENT_POLOZENO WHERE INDEKS = 20150320;
SELECT * FROM DA.PREDMET_POLOZENO WHERE IDPREDMET = 2010;
...
SELECT * FROM DA.STUDENT_POLOZENO WHERE INDEKS = 20152003;
SELECT * FROM DA.PREDMET_POLOZENO WHERE IDPREDMET = 1695;
