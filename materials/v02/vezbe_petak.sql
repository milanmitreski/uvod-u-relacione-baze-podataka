...
-- 5. Prikazati podatke o studentima i njihovim ispitima koji
-- su odrzani 28.01.2016. Izdvojiti indeks, ime i prezime studenta,
-- idpredmeta i ocenu

SELECT D.INDEKS /* I.INDEKS */,
       D.IME, D.PREZIME, I.IDPREDMETA, I.OCENA
FROM DA.DOSIJE D JOIN DA.ISPIT I ON (D.INDEKS = I.INDEKS)
WHERE I.DATPOLAGANJA = '2016-01-28';

-- 6. Izdvojiti podatke o polozenim ispitima. Prikazati indeks,
-- ime i prezime studenta koji je polozio ispit, naziv polozenog
-- predmeta i ocenu
...
-- 7. Izdvojiti podatke o studentima za koje vazi da su
-- diplomirali dana kada je odrzan neki ispit
...
-- Izdvojiti oznaek predmeta i broj espb bodova
...
-- malo slovo a na 4. poziciji i zavrsava se na malo slovo c i koji
-- su neki predmet ciji je broj espb bodova izmedju 2 i 10 polozili sa
-- ocenom 6, 8 ili 10 izmedju 5. januara 2018 i 15. decembra 2018.
-- Rezultat urediti prema prezimenu u rastucem poretku i imenu
...
-- 10. Za svaki predmet koji moze da se slusa na nekom studijskom
-- programu izdvojiti uslovne predmete tog predmeta. Izdvojiti
-- identifikator studijskog programa, identifikator predmeta, vrstu
-- tog predmeta (obavezni ili izborni) na studijskom programu i
-- identifikator uslovnog predmeta. Izdvojiti i predmete koji
-- nemaju uslovne predmete

SELECT PP.IDPROGRAMA, PP.IDPREDMETA, PP.VRSTA, UP.IDUSLOVNOGPREDMETA
FROM DA.PREDMETPROGRAMA PP LEFT JOIN DA.USLOVNIPREDMET UP ON (
        PP.IDPROGRAMA = UP.IDPROGRAMA AND
        PP.IDPREDMETA = UP.IDPREDMETA
    );
